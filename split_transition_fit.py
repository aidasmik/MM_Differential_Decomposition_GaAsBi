...
def main() -> None:
    ...
    if not args.no_feature_scan:
        ...
        export.write_feature_csv(output_dir / "feature_candidates.csv", feature_scan)
        ...
        export.write_splitting_csv(
            output_dir / "valence_band_splitting.csv",
            splitting_estimates,
        )
        export.write_consensus_csv(
    ...
    export.write_short_report_csv(output_dir / "short_report.csv", report_rows)
    summary_path = output_dir / "split_transition_fit_summary.json"
    summary_path.write_text(
        json.dumps(export.json_ready(summary), indent=2),
        encoding="utf-8",
    )
    export.write_fit_csv(output_dir / "split_transition_fit_curve.csv", fit)
