...
| --- | ---: | --- | --- |
| Eg change per Bi% | -85.213 | meV/%Bi | Linear fit |
| Delta Vb change per Bi% | 13.052 | meV/%Bi | Linear fit |
| Mean Delta Vb | 63.572 | meV | Accepted selected values |
| Mean Eg | 1.10578 | eV | Accepted selected values |

### Interpretation Notes

- `Delta Vb` is reported as the split fundamental-edge separation `E0,2 - E0,1` for the selected candidate.
- The local LNFK abstract in this workspace reports ordered GaAsBi `Delta Vb` values around `40-100 meV` for `x ~ 0.02-0.08`, with the splitting approximately temperature independent.
- Use the `selected_delta_*` columns in `selected_results.csv` to audit whether a saved value came from LD, LB, KK, dD/dE, or a manual override.
